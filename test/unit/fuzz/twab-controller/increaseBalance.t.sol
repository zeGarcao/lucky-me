...
contract IncreaseBalance_Unit_Fuzz_Test is TwabController_Unit_Shared_Test {
    address user;
    // ================================== SETUP MODIFIERS ==================================
    ...
    function testFuzz_IncreaseBalance_ZeroCardinality(uint256 _user, uint256 _increaseAmount)
        public
        ...
        whenCardinalityIsZero
    {
        ...
        emit BalanceIncreased(user, increaseAmount, increaseAmount, block.timestamp);
        ...
        // Increase balance
        uint256 newBalance = twabController.increaseBalance(user, increaseAmount);
    ...
    function testFuzz_IncreaseBalance_NewPeriodWithCardinalityBelowMax(
        uint256 _user,
        ...
        // Expect the `BalanceIncreased` event to be emitted
        vm.expectEmit(true, true, true, true);
        emit BalanceIncreased(user, increaseAmount, expectedNewBalance, block.timestamp);
    ...
    function testFuzz_IncreaseBalance_NotNewPeriodWithCardinalityBelowMax(
        uint256 _user,
        uint256 _increaseAmount,
        uint256 _cardinality,
        uint256 _skip
    ...
    function testFuzz_IncreaseBalance_NewPeriodWithMaxCardinality(uint256 _user, uint256 _increaseAmount, uint256 _skip)
        public
        setUpState(_user, _increaseAmount)
        whenMaxCardinality
        whenNewPeriod(_skip)
