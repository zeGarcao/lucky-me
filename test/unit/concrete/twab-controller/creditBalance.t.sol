...
contract CreditBalance_Unit_Concrete_Test is TwabController_Unit_Shared_Test {
    // ================================== SETUP MODIFIERS ==================================
    ...
    modifier whenCardinalityBelowMax() {
        twabController.creditBalance(bob, increaseAmount);
        _;
    }
    ...
    function test_RevertWhen_AmountIsZero() public whenOwner whenAmountIsZero {
        // Expect revert with `TWAB_CREDIT_BALANCE__INVALID_AMOUNT` error
        vm.expectRevert(TWAB_CREDIT_BALANCE__INVALID_AMOUNT.selector);
        twabController.creditBalance(bob, increaseAmount);
    }

    // ==================================== HAPPY TESTS ====================================

    function test_CreditBalance_ZeroCardinality() public whenOwner whenAmountIsNotZero whenCardinalityIsZero {
        // Expect the `BalanceCredited` event to be emitted
        vm.expectEmit(true, true, true, true);
        emit BalanceCredited(bob, increaseAmount, increaseAmount, block.timestamp);
        ...
        // Increase balance
        uint256 newBalance = twabController.creditBalance(bob, increaseAmount);
    ...
    function test_CreditBalance_NewPeriodWithCardinalityBelowMax()
        public
        whenOwner
        whenAmountIsNotZero
        ...
        whenNewPeriod
    {
        ...
        // Expect the `BalanceCredited` event to be emitted
        vm.expectEmit(true, true, true, true);
        emit BalanceCredited(bob, increaseAmount, expectedNewBalance, block.timestamp);
    ...
    function test_CreditBalance_NewPeriodWithMaxCardinality()
        public
        whenOwner
        whenAmountIsNotZero
        whenMaxCardinality
